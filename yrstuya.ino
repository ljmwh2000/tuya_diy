...
#define PIXEL_PIN    6  // Digital IO pin connected to the NeoPixels. 数字引脚6连接到像素输出

#define PIXEL_COUNT 16  // Number of NeoPixels  灯的个数

// Declare our NeoPixel strip object:  声明一个灯带对像
Adafruit_NeoPixel strip(PIXEL_COUNT, PIXEL_PIN, NEO_GRB + NEO_KHZ800);
// Argument 1 = Number of pixels in NeoPixel strip  参数1 LED灯的个数
// Argument 2 = Arduino pin number (most are valid) 参数2 arduino引脚编号
// Argument 3 = Pixel type flags, add together as needed: 参数3 LED类型标志，根据需要添加在一起
//   NEO_KHZ800  800 KHz bitstream (most NeoPixel products w/WS2812 LEDs)   800KHZ比特流，适用2812（大多数新像素产品如WS2812 LEDs)
//   NEO_KHZ400  400 KHz (classic 'v1' (not v2) FLORA pixels, WS2811 drivers)
//   NEO_GRB     Pixels are wired for GRB bitstream (most NeoPixel products) LED 颜色顺序
//   NEO_RGB     Pixels are wired for RGB bitstream (v1 FLORA pixels, not v2)
//   NEO_RGBW    Pixels are wired for RGBW bitstream (NeoPixel RGBW products)

boolean oldState = HIGH;
int     mode     = 0;    // Currently-active animation mode, 0-9   当前模式


TuyaWifi my_device;   //生成一个涂鸦对象

unsigned char led_state = 0;
/* Connect network button pin */

int key_pin = 7;     //S3  连接到arduino的7，配网用
...
unsigned char pid[] = {"9g2ztbcbmtbwuguj"};//*********处替换成涂鸦IoT平台自己创建的产品的PID
...
void loop() {
  ...
  //Enter the connection network mode when Pin7 is pressed.  当D7脚的按扭按下时（低电平）进入配网模式
  if (digitalRead(key_pin) == LOW) {
    delay(80);
    if (digitalRead(key_pin) == LOW) {
      my_device.mcu_set_wifi_mode(SMART_CONFIG);  //检测到配网按扭按下80MS进入配网模式
...
my_device.uart_service();
  ...
  /* LED blinks when network is being connected 当开始连网时 13脚的LED灯闪烁*/
  ...
  // Get current button state.得到按扭当前状态
  boolean newState = digitalRead(BUTTON_PIN);

  // Check if state changed from high to low (button press).  检测按扭是否按下，（从高电平变低电平）
  if((newState == LOW) && (oldState == HIGH)) {
    // Short delay to debounce button.
    delay(20);
    // Check if button is still low after debounce.  如果20MS后按键仍然按下
    newState = digitalRead(BUTTON_PIN);
    if(newState == LOW) {      // Yes, still low  是的，仍然按下
      if(++mode > 8) mode = 0; // Advance to next mode, wrap around after #8 轮到下一模式，到8时变为0，一共8种模式
...
//用颜色一个接一个地填充灯带像素。灯带未清除
//第一；那里的任何东西都会被逐像素覆盖。传递颜色
//（作为单个“压缩”32位值，您可以通过调用//strip.Color（红、绿、蓝），如上面的loop（）函数所示，
//像素之间的延迟时间（毫秒）。

//void colorWipe（uint32\t color，int wait）
...
// a la strip.Color(r,g,b) as mentioned above), and a delay time (in ms)剧院天棚式追逐灯。传入颜色（32位值，la条带颜色（r、g、b）和延迟时间（ms）帧之间。
// between frames.
...
// Rainbow cycle along whole strip. Pass delay time (in ms) between frames.彩虹沿着整个灯带循环。帧之间的传递延迟时间（毫秒）。
void rainbow(int wait) {
  // Hue of first pixel runs 3 complete loops through the color wheel.第一个像素的色调通过色轮运行3个完整的循环。
  // Color wheel has a range of 65536 but it's OK if we roll over, so
  // just count from 0 to 3*65536. Adding 256 to firstPixelHue each time色轮的范围是65536，但如果我们翻车的话没关系，所以只需从0数到3*65536。每次向firstPixelHue添加256意味着我们要让3*65536/256=768通过这个外环：
...
// Rainbow-enhanced theater marquee. Pass delay time (in ms) between frames. 增强彩虹
...
unsigned char dp_process(unsigned char dpid,const unsigned char value[], unsigned short length)
{
  switch(dpid) {
    case DPID_SWITCH_LED:
      ...
      if (dp_bool_value) {
        //Turn on
        digitalWrite(LED_BUILTIN, HIGH);   //我加的，测试用
        colorfill (strip.Color(  0, 255,  0)); //上一次状态
      } else {
        //Turn off
        digitalWrite(LED_BUILTIN, LOW);   //我加的，测试用
    ...
    case DPID_WORK_MODE:
              ...
              dp_enum_value  = my_device.mcu_get_dp_download_data(dpid, value, length); /* Get the value of the down DP command */
                switch(dp_enum_value){
                        ...
                        break;
                        case 1: // colour mode  彩色模式
                        colorfill (strip.Color(  100 ,100,  100));  //我加的彩色模式
                        break;
                        case 2: // scene mode   场景模式
                         colorfill (strip.Color(  150 ,0,  150));  //我加的彩色模式
                        break;
                       case 3: // music mode   音乐模式
                        colorfill (strip.Color(  0 ,150,  150));  //我加的音乐模式
                        break;
...
//我的没有这个
    ...
    break;
     ...
     case DPID_DREAMLIGHT_SCENE_MODE: //炫彩情景
       ...
       //我的没有这个lightpixel_number_set

      case DPID_LIGHTPIXEL_NUMBER_SET: //长度设置
            //digitalWrite(LED_BUILTIN, HIGH);   //我加的，测试用
          Cang_Val = my_device.mcu_get_dp_download_data(DPID_LIGHTPIXEL_NUMBER_SET, value, length); 
          my_device.mcu_dp_update(DPID_LIGHTPIXEL_NUMBER_SET, Cang_Val, length); //// 
          //colorfill (strip.Color(  255, 255,  255));  //白光
            
          strip.updateLength(Cang_Val);                
          strip.show();                                  
      ...
      case DPID_BRIGHT_VALUE://亮度
          Bright_Val = my_device.mcu_get_dp_download_data(DPID_BRIGHT_VALUE, value, length);
          strip.setBrightness(Bright_Val);
          strip.show(); 
          my_device.mcu_dp_update(dpid, Bright_Val, length);
      break;      


            //唤醒 DPID_WAKEUP_MODE//   按设定的时间淡出直至全亮
            //注意手机调试时，必须在框中输入20以上的值，这是经验
      case DPID_WAKEUP_MODE://亮度
          ...
          for(Bright_Val=10;Bright_Val<255;Bright_Val++)
          {
     ...
     //入睡   //注意手机调试时，必须在框中输入20以上的值，这是经验
     case DPID_SLEEP_MODE://sleep mode   按设定的时间淡出直至熄灭
                    
          colorfill (strip.Color(  0, 0,  255));
          //strip.show();
          my_device.mcu_dp_update(dpid, value, length);
          Bright_Val = 200; //
          for(Bright_Val;Bright_Val>0;Bright_Val--)
          {
          ...
          colorfill (strip.Color(  0, 0,  0)); //关灯
     break;           
